...
class BlittForConditionalGenerationModelOutput(ModelOutput):
	loss: Optional[tuple[torch.FloatTensor]] = None
	logits: Optional[tuple[torch.FloatTensor]] = None
	vision_outputs: Optional[torch.FloatTensor] = None
	point_outputs: Optional[torch.FloatTensor] = None # might remove vision and point and replace with "fused"?
	...
	def to_tuple(self) -> tuple[Any]:
		return tuple(
			self[k]
			if k not in ["vision_outputs", "point_outputs", "qformer_outputs", "language_model_outputs"]
			else getattr(self, k).to_tuple()
			for k in self.keys()
		)

class BlittImageTextMatchingModelOutput(ModelOutput):
	loss: Optional[torch.FloatTensor] = None
	logits_per_image: Optional[torch.FloatTensor] = None
	logits_per_text: Optional[torch.FloatTensor] = None
	text_embeds: Optional[torch.FloatTensor] = None
	image_embeds: Optional[torch.FloatTensor] = None
	image_embeds: Optional[torch.FloatTensor] = None # might remove vision and point and replace with "fused"?
	text_model_output: BaseModelOutputWithPooling = None
	vision_model_output: BaseModelOutputWithPooling = None
	point_model_output: BaseModelOutputWithPooling = None

	def to_tuple(self) -> tuple[Any]:
		return tuple(
			self[k] if k not in ["point_model_output", "text_model_output", "vision_model_output"] else getattr(self, k).to_tuple()
			for k in self.keys()
		)


class BlittTextModelOutput(ModelOutput):
	text_embeds: Optional[torch.FloatTensor] = None
	last_hidden_state: Optional[torch.FloatTensor] = None
	hidden_states: Optional[tuple[torch.FloatTensor, ...]] = None
	attentions: Optional[tuple[torch.FloatTensor, ...]] = None

class BlittVisionModelOutput(ModelOutput):#- - - - - - - - - - - - -|
	image_embeds: Optional[torch.FloatTensor] = None#				|
	last_hidden_state: Optional[torch.FloatTensor] = None#			|
	hidden_states: Optional[tuple[torch.FloatTensor, ...]] = None#	| - May remove for BlittFusedOutput?
	attentions: Optional[tuple[torch.FloatTensor, ...]] = None#		|
#																	|
class BlittPointModelOutput(ModelOutput):# - - - - - - - - - - - - -|
	Point_embeds: Optional[torch.FloatTensor] = None
	last_hidden_state: Optional[torch.FloatTensor] = None
	hidden_states: Optional[tuple[torch.FloatTensor, ...]] = None
	attentions: Optional[tuple[torch.FloatTensor, ...]] = None

class BlittVisionEmbeddings(nn.Module): #NEED TO EDIT
	def __init__(self, config: BlittVisionConfig):
		super().__init__()
		self.config = config
...
class BlittAttention(nn.Module):
...
# Copied from transformers.models.blip.modeling_blip.BlipEncoderLayer with Blip->Blitt
class BlittEncoderLayer(GradientCheckpointingLayer):
	def __init__(self, config: BlittConfig):
		super().__init__()
		self.embed_dim = config.hidden_size
		self.self_attn = BlittAttention(config)
		self.layer_norm1 = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_eps)
		self.mlp = BlittMLP(config)
...
class BlittPreTrainedModel(PreTrainedModel):
	config: BlittConfig
	...
	_no_split_modules = [
		"BlittAttention",
		"BlittQFormerMultiHeadAttention",
		"BlittEncoderLayer",
		"BlittTextEmbeddings",
		"T5Block",
		"OPTDecoderLayer",
	...
	def _init_weights(self, module):
		...
		if isinstance(module, (nn.Linear, nn.Conv2d)):
		...
		elif isinstance(module, BlittVisionEmbeddings):
			...
			(
				BlittModel,
				BlittTextModelWithProjection,
				BlittVisionModelWithProjection,
				BlittForConditionalGeneration,
				BlittForImageTextRetrieval,
			),
		):
			module.query_tokens.data.zero_()

# Copied from transformers.models.blip.modeling_blip.BlipVisionModel with Blip->Blitt, BLIP->BLIP_2
class BlittVisionModel(BlittPreTrainedModel):
	main_input_name = "pixel_values"
	config: BlittVisionConfig
	...
	def __init__(self, config: BlittVisionConfig):
		...
		self.embeddings = BlittVisionEmbeddings(config)
		self.encoder = BlittEncoder(config)
...
class BlittQFormerMultiHeadAttention(nn.Module):
...
# Copied from transformers.models.bert.modeling_bert.BertSelfOutput with Bert->BlittQFormer
class BlittQFormerSelfOutput(nn.Module):
	def __init__(self, config):
		super().__init__()
...
class BlittQFormerAttention(nn.Module):
	def __init__(self, config, is_cross_attention=False):
		super().__init__()
		self.attention = BlittQFormerMultiHeadAttention(config, is_cross_attention)
		self.output = BlittQFormerSelfOutput(config)
...
# Copied from transformers.models.bert.modeling_bert.BertIntermediate with Bert->BlittQFormer
class BlittQFormerIntermediate(nn.Module):
	def __init__(self, config):
		super().__init__()
...
# Copied from transformers.models.bert.modeling_bert.BertOutput with Bert->BlittQFormer
class BlittQFormerOutput(nn.Module):
	def __init__(self, config):
		super().__init__()
...
class BlittQFormerLayer(GradientCheckpointingLayer):
	def __init__(self, config, layer_idx):
		super().__init__()
		self.chunk_size_feed_forward = config.chunk_size_feed_forward
		self.seq_len_dim = 1
		self.attention = BlittQFormerAttention(config)

		self.layer_idx = layer_idx

		if layer_idx % config.cross_attention_frequency == 0:
			self.crossattention = BlittQFormerAttention(config, is_cross_attention=True)
			self.has_cross_attention = True
		else:
			self.has_cross_attention = False

		if config.use_qformer_text_input:
			self.intermediate = BlittQFormerIntermediate(config)
			self.output = BlittQFormerOutput(config)

		self.intermediate_query = BlittQFormerIntermediate(config)
		self.output_query = BlittQFormerOutput(config)
...
class BlittQFormerEncoder(nn.Module):
	def __init__(self, config):
		super().__init__()
		self.config = config
		self.layer = nn.ModuleList(
			[BlittQFormerLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
		)
...
class BlittTextEmbeddings(nn.Module):
...
class BlittQFormerModel(BlittPreTrainedModel):
	...
	def __init__(self, config: BlittQFormerConfig):
		...
		self.encoder = BlittQFormerEncoder(config)
	...
	def forward(
		...
		# `BlittQFormerModel` is kept as fp32
...
class BlittModel(BlittPreTrainedModel):
	config: BlittConfig
	main_input_name = "pixel_values"
	...
	def __init__(self, config: BlittConfig):
		...
		self.vision_model = BlittVisionModel._from_config(config.vision_config)
		self.point_model = BlittVisionModel._from_config(config.vision_config)

		self.query_tokens = nn.Parameter(torch.zeros(1, config.num_query_tokens, config.qformer_config.hidden_size))
		self.qformer = BlittQFormerModel._from_config(config.qformer_config)
	...
	def get_text_features(
		...
		**kwargs#: Unpack[TransformersKwargs],
		...
		```python
		>>> import torch
		>>> from transformers import AutoTokenizer, BlittModel

		>>> model = BlittModel.from_pretrained("Salesforce/Blitt-opt-2.7b")

		>>> tokenizer = AutoTokenizer.from_pretrained("Salesforce/Blitt-opt-2.7b")
	...
	def get_image_features(
		...
		>>> import requests
		>>> from transformers import AutoProcessor, BlittModel

		>>> model = BlittModel.from_pretrained("Salesforce/Blitt-opt-2.7b")

		>>> processor = AutoProcessor.from_pretrained("Salesforce/Blitt-opt-2.7b")
	...
	def get_qformer_features(
		...
		>>> from PIL import Image
		>>> import requests
		>>> from transformers import BlittProcessor, BlittModel

		>>> processor = BlittProcessor.from_pretrained("Salesforce/Blitt-opt-2.7b")
		>>> model = BlittModel.from_pretrained("Salesforce/Blitt-opt-2.7b")
	...
	def forward(
		...
		interpolate_pos_encoding: bool = False,
		**kwargs#: Unpack[TransformersKwargs],
	) -> Union[tuple, BlittForConditionalGenerationModelOutput]:
		r"""
		input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
			Indices of input sequence tokens in the vocabulary of the language model. Input tokens can optionally be
			provided to serve as text prompt, which the language model can continue.

			Indices can be obtained using [`BlittProcessor`]. See [`BlittProcessor.__call__`] for details.
		...
		>>> processor = BlittProcessor.from_pretrained("Salesforce/Blitt-opt-2.7b")
		>>> model = BlittModel.from_pretrained("Salesforce/Blitt-opt-2.7b", torch_dtype=torch.float16)
		...
		return BlittForConditionalGenerationModelOutput(
			loss=loss,
			logits=logits,
...
class BlittTextModelWithProjection(BlittPreTrainedModel):
	...
	def __init__(self, config: BlittConfig):
		...
		self.query_tokens = nn.Parameter(torch.zeros(1, config.num_query_tokens, config.qformer_config.hidden_size))
		self.embeddings = BlittTextEmbeddings(config.qformer_config)
		self.qformer = BlittQFormerModel(config.qformer_config)
	...
	def forward(
		...
		output_hidden_states: Optional[bool] = None,
		return_dict: Optional[bool] = None,
	) -> Union[tuple, BlittTextModelOutput]:
		r"""
		Examples:

		```python
		>>> import torch
		>>> from transformers import AutoProcessor, BlittTextModelWithProjection

		>>> device = "cuda" if torch.cuda.is_available() else "cpu"

		>>> model = BlittTextModelWithProjection.from_pretrained(
		...     "Salesforce/Blitt-itm-vit-g", torch_dtype=torch.float16
		... )

		>>> model.to(device)  # doctest: +IGNORE_RESULT

		>>> processor = AutoProcessor.from_pretrained("Salesforce/Blitt-itm-vit-g")
...
class BlittVisionModelWithProjection(BlittPreTrainedModel):
	main_input_name = "pixel_values"
	_keep_in_fp32_modules = ["query_tokens", "qformer"]
	_supports_flash_attn = False  # because self.qformer does not support FA2
	...
	def forward(
		...
		output_hidden_states: Optional[bool] = None,
		return_dict: Optional[bool] = None,
	) -> Union[tuple, BlittVisionModelOutput]:
		r"""
		Examples:
		...
		>>> from PIL import Image
		>>> import requests
		>>> from transformers import AutoProcessor, BlittVisionModelWithProjection

		>>> device = "cuda" if torch.cuda.is_available() else "cpu"

		>>> processor = AutoProcessor.from_pretrained("Salesforce/Blitt-itm-vit-g")
		>>> model = BlittVisionModelWithProjection.from_pretrained(
		...     "Salesforce/Blitt-itm-vit-g", torch_dtype=torch.float16
		... )
		>>> model.to(device)  # doctest: +IGNORE_RESULT
...
class BlittForConditionalGeneration(BlittPreTrainedModel, GenerationMixin):
	config: BlittConfig
	main_input_name = "pixel_values"
	...
	def forward(
		...
		**kwargs#: Unpack[TransformersKwargs],
		...
		>>> from PIL import Image
		>>> import requests
		>>> from transformers import BlittProcessor, BlittForConditionalGeneration
		>>> import torch

		>>> device = "cuda" if torch.cuda.is_available() else "cpu"

		>>> processor = BlittProcessor.from_pretrained("Salesforce/Blitt-opt-2.7b")
		>>> model = BlittForConditionalGeneration.from_pretrained(
		...     "Salesforce/Blitt-opt-2.7b", load_in_8bit=True, device_map={"": 0}, torch_dtype=torch.float16
		...
		```python
		>>> model = BlittForConditionalGeneration.from_pretrained(
		...     "Salesforce/Blitt-opt-2.7b", load_in_8bit=True, device_map={"": 0}, torch_dtype=torch.bfloat16
		... )  # doctest: +IGNORE_RESULT
...
class BlittForImageTextRetrieval(BlittPreTrainedModel):
	main_input_name = "pixel_values"
	_keep_in_fp32_modules = ["query_tokens", "qformer"]
	_supports_flash_attn = False  # because self.qformer does not support FA2
	...
	def forward(
		...
		output_hidden_states: Optional[bool] = None,
		return_dict: Optional[bool] = None,
	) -> Union[tuple, BlittImageTextMatchingModelOutput]:
		r"""
		input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
			Indices of input sequence tokens in the vocabulary of the language model. Input tokens can optionally be
			provided to serve as text prompt, which the language model can continue.
		...
		>>> from PIL import Image
		>>> import requests
		>>> from transformers import AutoProcessor, BlittForImageTextRetrieval

		>>> device = "cuda" if torch.cuda.is_available() else "cpu"

		>>> model = BlittForImageTextRetrieval.from_pretrained("Salesforce/Blitt-itm-vit-g", torch_dtype=torch.float16)
		>>> processor = AutoProcessor.from_pretrained("Salesforce/Blitt-itm-vit-g")
		...
		return BlittImageTextMatchingModelOutput(
